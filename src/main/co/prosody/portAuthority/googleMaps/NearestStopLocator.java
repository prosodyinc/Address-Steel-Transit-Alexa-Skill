...
public class NearestStopLocator {
    ...
    protected static Stop process(Location source, String routeID, String direction) throws IOException, JSONException, InvalidInputException {
        ...
        Stop nearestStop = findNearestStop(source, listOfStops);
     ...
     * @throws IOException
     */
    public static Stop findNearestStop(Location source, List<Stop> stops) throws JSONException, IOException {
        ...
        while (currentbatch < numberOfBatches) {
            ...
            String url = DISTANCE_MATRIX_URL + "?origins=" + source.getLat() + "," + source.getLng() + "&destinations=" + destinationString.substring(1) + "&key=" + GOOGLE_MAPS_KEY + "&units=imperial&mode=walk&transit_mode=walking";
            log.info("DISTANCE MATRIX URL: " + url);
            Stop batchNearest = calculateShortestDistance(url, currentBatchStops);
            log.info("{},{},{}:{},{}:{},{}", batchNearest.getStopName(), batchNearest.getDistance(), source.getLat(), source.getLng() , batchNearest.getLatitude(), batchNearest.getLongitude());
